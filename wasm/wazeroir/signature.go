...
func wasmOpcodeSignature(f *wasm.FunctionInstance, op wasm.Opcode, index uint32) (*signature, error) {
	...
	case wasm.OpcodeCall:
		return funcTypeToSignature(f.ModuleInstance.Functions[index].FunctionType), nil
	case wasm.OpcodeCallIndirect:
		ret := funcTypeToSignature(f.ModuleInstance.Types[index].FunctionType)
	...
	case wasm.OpcodeLocalGet:
		...
		var t UnsignedType
		if index < inputLen {
			t = wasmValueTypeToUnsignedType(f.FunctionType.Params[index])
		} else {
