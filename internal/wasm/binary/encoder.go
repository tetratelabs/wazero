...
func EncodeModule(m *wasm.Module) (bytes []byte) {
	...
	if m.SectionElementCount(wasm2.SectionIDType) > 0 {
		bytes = append(bytes, encodeTypeSection(m.TypeSection)...)
	}
	if m.SectionElementCount(wasm2.SectionIDImport) > 0 {
		bytes = append(bytes, encodeImportSection(m.ImportSection)...)
	}
	if m.SectionElementCount(wasm2.SectionIDFunction) > 0 {
		bytes = append(bytes, encodeFunctionSection(m.FunctionSection)...)
	}
	if m.SectionElementCount(wasm2.SectionIDTable) > 0 {
		panic("TODO: TableSection")
	}
	if m.SectionElementCount(wasm2.SectionIDMemory) > 0 {
		bytes = append(bytes, encodeMemorySection(m.MemorySection)...)
	}
	if m.SectionElementCount(wasm2.SectionIDGlobal) > 0 {
		panic("TODO: GlobalSection")
	}
	if m.SectionElementCount(wasm2.SectionIDExport) > 0 {
		bytes = append(bytes, encodeExportSection(m.ExportSection)...)
	}
	if m.SectionElementCount(wasm2.SectionIDStart) > 0 {
		bytes = append(bytes, encodeStartSection(*m.StartSection)...)
	}
	if m.SectionElementCount(wasm2.SectionIDElement) > 0 {
		panic("TODO: ElementSection")
	}
	if m.SectionElementCount(wasm2.SectionIDCode) > 0 {
		bytes = append(bytes, encodeCodeSection(m.CodeSection)...)
	}
	if m.SectionElementCount(wasm2.SectionIDData) > 0 {
		panic("TODO: DataSection")
	}
	if m.SectionElementCount(wasm2.SectionIDCustom) > 0 {
		...
		if m.NameSection != nil {
			...
			bytes = append(bytes, encodeSection(wasm2.SectionIDCustom, nameSection)...)
		}
	}
	return
}
