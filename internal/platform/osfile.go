...
func newDefaultOsFile(openPath string, openFlag int, openPerm fs.FileMode, f *os.File) File {
...
func (f *osFile) IsNonblock() bool {
	return f.flag&O_NONBLOCK == O_NONBLOCK
}
...
func (f *osFile) SetNonblock(enable bool) (errno syscall.Errno) {
	...
	if err := setNonblock(f.file.Fd(), enable); err != nil {
		return fileError(f, f.closed, UnwrapOSError(err))
	}
	return 0
}
...
func (f *osFile) Stat() (Stat_t, syscall.Errno) {
	if f.closed {
		return Stat_t{}, syscall.EBADF
	}
...
func (f *osFile) Readdir(n int) (dirents []Dirent, errno syscall.Errno) {
...
func (f *osFile) Truncate(size int64) (errno syscall.Errno) {
	if errno = UnwrapOSError(f.file.Truncate(size)); errno != 0 {
		// Defer validation overhead until we've already had an error.
		errno = fileError(f, f.closed, errno)
	}
	return
...
func (f *osFile) Chmod(mode fs.FileMode) syscall.Errno {
	...
	return UnwrapOSError(f.file.Chmod(mode))
}
