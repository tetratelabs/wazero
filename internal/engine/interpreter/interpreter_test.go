		...
		t.Run(wasm.MiscInstructionName(tc.op), func(t *testing.T) {
			...
			for i := 0; i < casenum; i++ {
				...
				t.Run(strconv.Itoa(i), func(t *testing.T) {
					var body []*IrOp
					if in32bit {
						body = append(body, &IrOp{
							KindOp: wazeroir.OperationKindConstF32,
							U1:     uint64(math.Float32bits(tc.input32bit[i])),
						})
					} else {
						body = append(body, &IrOp{
							KindOp: wazeroir.OperationKindConstF64,
							U1:     uint64(math.Float64bits(tc.input64bit[i])),
						})
					}

					body = append(body, &IrOp{
						KindOp: wazeroir.OperationKindITruncFromF,
					...
					body = append(body,
						&IrOp{KindOp: wazeroir.OperationKindBr, U1: uint64(math.MaxUint64)},
					)
	...
	t.Run("32bit", func(t *testing.T) {
		...
		for _, tt := range tests {
			...
			t.Run(fmt.Sprintf("%s(i32.const(0x%x))", wasm.InstructionName(tc.opcode), tc.in), func(t *testing.T) {
				...
				f := &function{
					moduleInstance: &wasm.ModuleInstance{Engine: &moduleEngine{}},
					parent: &code{body: []*IrOp{
						{KindOp: wazeroir.OperationKindConstI32, U1: uint64(uint32(tc.in))},
						{KindOp: translateToIROperationKind(tc.opcode)},
						{KindOp: wazeroir.OperationKindBr, U1: uint64(math.MaxUint64)},
					}},
				}
				ce.callNativeFunc(testCtx, &wasm.ModuleInstance{}, f)
		...
		for _, tt := range tests {
			...
			t.Run(fmt.Sprintf("%s(i64.const(0x%x))", wasm.InstructionName(tc.opcode), tc.in), func(t *testing.T) {
				...
				f := &function{
					moduleInstance: &wasm.ModuleInstance{Engine: &moduleEngine{}},
					parent: &code{body: []*IrOp{
						{KindOp: wazeroir.OperationKindConstI64, U1: uint64(tc.in)},
						{KindOp: translateToIROperationKind(tc.opcode)},
						{KindOp: wazeroir.OperationKindBr, U1: uint64(math.MaxUint64)},
					}},
				}
