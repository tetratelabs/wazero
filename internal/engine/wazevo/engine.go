...
type (
	...
	sharedFunctions struct {
		...
		// adjustExnrefsAddress is the address of the trampoline that adjusts a call's exnref
		// reference counts.
		adjustExnrefsAddress *byte
...
func (e *engine) compileSharedFunctions() {
	...
	addTrampoline(8,
		e.machine.CompileGoFunctionTrampoline(wazevoapi.ExitCodeThrowAlloc, &ssa.Signature{
			// exec context, tag index → params buffer
	...
	e.be.Init()
	addTrampoline(16,
		e.machine.CompileGoFunctionTrampoline(wazevoapi.ExitCodeAdjustExnrefs, &ssa.Signature{
			// exec context, handle gaining a reference, handle losing one
			Params:  []ssa.Type{ssa.TypeI64, ssa.TypeI64, ssa.TypeI64},
			Results: []ssa.Type{},
		}, false))
	...
	offset += sizes[15]
	fns.adjustExnrefsAddress = &fns.executable[offset]
