...
func TestStackUnwinder(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			buf := make([]byte, len(tc.contents)*8)
			for i, v := range tc.contents {
				binary.LittleEndian.PutUint64(buf[i*8:], v)
			}
			returnAddresses := StackUnwinder(uintptr(unsafe.Pointer(&buf[0])),
				uintptr(unsafe.Pointer(&buf[len(buf)-1])))
