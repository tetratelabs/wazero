...
func (m *machine) LowerInstr(instr *ssa.Instruction) {
	...
	case ssa.OpcodeVImin:
		...
		rm := m.getOperand_NR(m.compiler.ValueDefinition(y), extModeNone)
		rd := operandNR(m.compiler.VRegOf(instr.Return()))
		sub.asVecRRR(vecOpSmin, rd, rn, rm, arr)
		m.insert(sub)
	case ssa.OpcodeVUmin:
		...
		rm := m.getOperand_NR(m.compiler.ValueDefinition(y), extModeNone)
		rd := operandNR(m.compiler.VRegOf(instr.Return()))
		sub.asVecRRR(vecOpUmin, rd, rn, rm, arr)
		m.insert(sub)
	case ssa.OpcodeVImax:
		...
		rm := m.getOperand_NR(m.compiler.ValueDefinition(y), extModeNone)
		rd := operandNR(m.compiler.VRegOf(instr.Return()))
		sub.asVecRRR(vecOpSmax, rd, rn, rm, arr)
		m.insert(sub)
	case ssa.OpcodeVUmax:
		...
		rm := m.getOperand_NR(m.compiler.ValueDefinition(y), extModeNone)
		rd := operandNR(m.compiler.VRegOf(instr.Return()))
		sub.asVecRRR(vecOpUmax, rd, rn, rm, arr)
		m.insert(sub)
	case ssa.OpcodeVAvgRound:
		...
		rm := m.getOperand_NR(m.compiler.ValueDefinition(y), extModeNone)
		rd := operandNR(m.compiler.VRegOf(instr.Return()))
		sub.asVecRRR(vecOpUrhadd, rd, rn, rm, arr)
		m.insert(sub)
