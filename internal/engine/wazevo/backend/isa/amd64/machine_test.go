...
func TestMachine_lowerXmmRmR(t *testing.T) {
	tests := []struct {
		name   string
		rn, rm operand
		rd     regalloc.VReg
		...
		{
			name: "reg operands",
			rn:   newOperandReg(xmm0VReg),
			rm:   newOperandReg(xmm1VReg),
			rd:   xmm15VReg,
			tpe:  ssa.TypeF64,
			exp: `
	movdqu %xmm0, %xmm1?
...
`,
		...
		{
			name: "mem, reg operands",
			rn:   newOperandMem(newAmodeImmReg(123, raxVReg)),
			rm:   newOperandReg(xmm1VReg),
			rd:   xmm15VReg,
			tpe:  ssa.TypeF64,
			exp: `
...
`,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			_, _, m := newSetupWithMockContext()
			m.lowerXmmRmR(sseOpcodeAddss, tc.rn, tc.rm, tc.rd, tc.tpe)
			m.ectx.FlushPendingInstructions()
			m.ectx.RootInstr = m.ectx.PerBlockHead
			require.Equal(t, tc.exp, m.Format())
		})
