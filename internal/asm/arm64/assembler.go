...
type Assembler interface {
	asm.AssemblerBase
	// CompileJumpToMemory adds jump-type instruction whose destination is stored in the memory address specified by `baseReg`,
	// and returns the corresponding Node in the assembled linked list.
	//
	// Note: this has exactly the same implemnetation as AssemblerBase.CompileJumpToRegister in the homemade assembler.
	// Therefore, this will be removed after golang-asm removal.
