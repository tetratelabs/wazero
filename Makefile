...
.PHONY: build.bench
build.bench:
	tinygo build -o bench/case/case.wasm -gc=leaking -scheduler=none -target=wasi bench/case/case.go

.PHONY: build.examples
build.examples:
	@find ./examples/wasm -type f -name "*.go" | xargs -Ip /bin/sh -c 'tinygo build -o $$(echo p | sed -e 's/\.go/\.wasm/') -gc=leaking -scheduler=none -target=wasi p'
